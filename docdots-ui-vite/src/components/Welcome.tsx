...
export default function Welcome({ onUpload, persona, setPersona, job, setJob }: Props) {
      ...
      <div className="p-10 flex flex-col justify-center gap-6 bg-gradient-to-br from-indigo-50 to-white">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="space-y-1">
            <div className="text-xs font-semibold text-slate-500">Persona</div>
            <input
              className="h-10 w-full rounded-xl border border-slate-200 px-3 text-sm focus:ring-2 focus:ring-indigo-300"
              value={persona}
              onChange={(e) => setPersona(e.target.value)}
              placeholder="e.g., Food lover"
            />
          </div>
          <div className="space-y-1">
            <div className="text-xs font-semibold text-slate-500">Job to be done</div>
            <input
              className="h-10 w-full rounded-xl border border-slate-200 px-3 text-sm focus:ring-2 focus:ring-indigo-300"
              value={job}
